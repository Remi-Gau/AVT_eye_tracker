function [EyeData] = TobiiEyeXTrackCalib(EyeData, Opt)
...
for iTrial = 1:floor(numel(TrialsStarts)/2)
    ...
    if Time(IndexTimeEnd)-Time(IndexTimeStart)<CalibDur-500 || ...
            Time(IndexTimeEnd)-Time(IndexTimeStart)>CalibDur+PreCalibDur+100
        ...
        if numel(Samples)>SamplingFreq/2 %We make sure that we have at least more than half a second worth of data
            ...
            if mad(X,1)<PPD && mad(Y,1)<PPD
                ...
                errorbar(median(X), median(Y), mad(Y,1), mad(Y,1), mad(X,1), mad(X,1), 'or')
